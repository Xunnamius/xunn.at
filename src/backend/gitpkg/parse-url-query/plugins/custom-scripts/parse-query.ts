...
export function queryKeyIsCustomScript(key: string | symbol | number): key is string {
  return typeof key === 'string' && key.startsWith(SCRIPTS_PREFIX);
}

function trimAndAnd(v: string) {
  return v.slice(v.startsWith('&&') ? 2 : 0, v.endsWith('&&') ? -2 : undefined).trim();
}
...
export function parseQueryAsCustomScript(
  ...
  if (typeof value === 'string') {
    const v = value.trim();
    type = v.startsWith('&&') ? 'append' : v.endsWith('&&') ? 'prepend' : 'replace';
    script = trimAndAnd(v);
  } else {
    const s = [];
