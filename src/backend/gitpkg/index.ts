...
export async function handlePackageRequest({
  requestUrl,
  query,
  parseFromUrl,
  response
  ...
  try {
    const pkgOpts = getDefaultParser(parseFromUrl).parse(requestUrl || '', query);
    const { commitIshInfo: cii } = pkgOpts;

    response.status(200);
    response.setHeader(
      'Content-Disposition',
      `attachment; filename="${[cii.user, cii.repo, ...(cii.subdirs || []), cii.commit]
        .filter(Boolean)
        .join('-')}.tgz"`
    );
